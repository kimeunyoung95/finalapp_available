...
public class CaldroidGridAdapter extends BaseAdapter {
    ...
    private void getDefaultResources() {
        ...
        if (squareTextViewCell) {
            theme.resolveAttribute(com.caldroid.R.attr.styleCaldroidSquareCell, styleCellVal, true);
        } else {
            theme.resolveAttribute(com.caldroid.R.attr.styleCaldroidNormalCell, styleCellVal, true);
        }

        // Get default background of cell
        TypedArray typedArray = wrapped.obtainStyledAttributes(styleCellVal.data, com.caldroid.R.styleable.Cell);
        defaultCellBackgroundRes = typedArray.getResourceId(com.caldroid.R.styleable.Cell_android_background, -1);
        defaultTextColorRes = typedArray.getColorStateList(com.caldroid.R.styleable.Cell_android_textColor);
        typedArray.recycle();
    }
